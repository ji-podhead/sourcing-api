...
async def create_camera(data: Dict[str, str]):
    ...
    try:
            ...
            if existing_id is not "Not in DB" and existing_id is not None:
                return get_gigE_camera(existing_id)
            else:
            ...
            new_camera_data = get_gigE_camera(new_camera_id)
...
async def get_camera(camera: int | str):
    ...
    data = get_gigE_camera(camera_id)
    if data is None:
        raise HTTPException(status_code=404, detail=f"Camera with ID '{camera_id}' not found.")
    return data

@router.get("/camera/{camera_id}/temperature")
async def get_camera_temperature(camera_id: int):
    """Returns the temperature for a given camera."""
    # In a real application, you would fetch this from the camera's features.
