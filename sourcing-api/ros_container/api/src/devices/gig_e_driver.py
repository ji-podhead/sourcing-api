...
import signal
import sys
class ThreadSafeRosClient(roslibpy.Ros):
...
class GigECameraNode:
    def __init__(self, camera_id: int, ros_host: str = 'localhost', ros_port: int = 9090):
        ...
        self.ros_client = roslibpy.Ros(host=ros_host, port=ros_port)#, install_signal_handlers=False)
        self.ros_client = ThreadSafeRosClient(host=ros_host, port=ros_port)
    ...
    def _on_ros_connect(self):
        ...
        try:
            self.set_feature_service = roslibpy.Service(
                self.ros_client, 
                self.set_feature_service_name, 
                'interfaces/srv/SetFeature'
            )
