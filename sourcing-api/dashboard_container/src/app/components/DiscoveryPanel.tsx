...
import { handleCreateCamera, handleAction } from '../redux/thunks';
                  ...
                  <tr key={index}>
                    ...
                    <td className="p-2 border-b">
                      {cam.is_in_db ? (
                        <>
                          <button
                            onClick={() => dispatch(handleAction({ endpoint: `/camera/${cam.name}/start`, successMessage: 'Camera started', errorMessage: 'Failed to start camera' }))}
                            className="bg-green-500 hover:bg-green-700 text-white font-bold py-1 px-3 rounded text-sm mr-2"
                          >
                            Start
                          </button>
                          <button
                            onClick={() => dispatch(handleAction({ endpoint: `/camera/${cam.name}/stop`, successMessage: 'Camera stopped', errorMessage: 'Failed to stop camera' }))}
