  ...
  useEffect(() => {
    if (selectedCamera) {
      const intervalId = setInterval(() => {
        dispatch(fetchCameraDetails(selectedCamera.id));
      }, refreshInterval);
      return () => clearInterval(intervalId);
    }
  }, [dispatch, selectedCamera, refreshInterval]);
    ...
    <div className="w-full h-full text-black bg-gray-100 p-8">
      ...
      <div className="w-full bg-white p-6 rounded-lg shadow-md mb-8">
        ...
        <div className="w-full h-96 border rounded-md">
          <iframe src="http://localhost:8081/?rosbridge-websocket-url=ws://localhost:9090" className="w-full h-full"></iframe>
        </div>
      </div>
    </div>
  );
